...
const MyBook = () => {
  ...
  const saveBtnClick = e => {
    ...
    if(state.book.imgs.length === 0 && state.newImgFiles.length === 0){
      imgsErrorElement = <li>you deleted all old images but you did not upload any new ones</li>
    }

    let pdfErrorElement = null
    if(state.newPdfFile === null && state.book.pdfUrl === ''){
      pdfErrorElement = <li>you deleted the old pdf file but you did not upload new one</li>
    }
        ...
        switch (data) {
          ...
          default:
            ...
            setState({
              ...
              modalTitle: 'updates successfully',
              modalElement: <p>the book is updated successfully</p>
            })
